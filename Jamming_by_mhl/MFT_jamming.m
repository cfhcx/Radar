function MFT_jamming=MFT_jamming(PRI,DutyRatio,JammingPower,B,Bd,fd0,fs,f,delta_R,R,n)
%多假目标欺骗 multiple false target fd0起始多普勒频率 Bd多普勒调制带宽 delta_R间隔距离 
c=3e8;
Tp=PRI*DutyRatio;
K=B/Tp;
Delay=2*R/c;
...
MFT_jamming=zeros(1,N);
for k=1:n
    fj=abs(fd0+Bd*randn(1))
    MFT_jamming(1,(k-1)*N/n+1:k*N/n)=[JammingPower*exp(1i*2*pi*((fj)*t_Tp+1/2*K*t_Tp.^2)) zeros(1,delta_N)];
...
% 
% s_com_j=conv(MFT_jamming,h); %线性调频信号经过匹配滤波器后的输出(时域卷积)
% s_com=conv(s,h);
% 
